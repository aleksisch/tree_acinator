...
int AkinatorTree::FreeTree(TreeNode* current)
{
...
int AkinatorTree::ObjectInfo(const char* name)
{
    ...
    GetNodeInfo(head_node, name, node_info);
...
int AkinatorTree::CompareNode(const char* first_name, const char* second_name)
{
    ...
    GetNodeInfo(head_node, first_name, first_info);

    Question* second_info = new Question[MAX_DEPTH];

    GetNodeInfo(head_node, second_name, second_info);
...
int AkinatorTree::play(TreeNode* current)
{
    if (current == nullptr)
    {
        printf("Ok, let's go\n");
